...
export default function AdminAuth() {
	...
	const submitAdminRegister = async () => {
		...
		try {
			const result = await axios.post(`${process.env.REACT_APP_API_URL}/Admin/register`, formData, {
				headers: {
	...
	const submitAdminLogin = async () => {
		...
		formData.append("password", password);
		try {
			const result = await axios.post(`${process.env.REACT_APP_API_URL}/Admin/login`, formData, {
				headers: {
			...
			<div className={containerCls ? "authContainer right-panel-active" : "authContainer"} id="container">
				<div className="form-container sign-up-container">
					...
					<h1>Admin Register</h1>
						...
						<CustomButton1 title="Sign Up" fun={submitAdminRegister} padding="12px 35px" />
					</div>
				</div>
				<div className="form-container sign-in-container">
					...
					<h1>Admin Login</h1>
						<CustomInput type="email" placeholder="Enter your email" fun={setEmail} />
						<CustomInput type="password" placeholder="Enter your password" fun={setPassword} />
						<CustomButton1 title="Sign In" fun={submitAdminLogin} padding="12px 35px" />
					</div>
				</div>
