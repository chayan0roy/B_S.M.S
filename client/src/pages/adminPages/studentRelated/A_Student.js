...
export default function A_Student() {
	...
	useEffect(() => {
		...
		} else if (batch && batch.studentDataList && batch.studentDataList.length !== 0) {
			Handle_Get_Students_All_Data();
		} else {
			dispatch(deleteStudentList());
		}
	}, [isStudent, batch, dispatch]);


	const Handle_Get_All_Students = async () => {
		...
		if (token && role && role === "Admin") {
			try {
				const result = await axios.post(
					`${process.env.REACT_APP_API_URL}/Admin/getAllStudents`,
					null,
					{
						headers: {
		...
		try {
			...
			if (token && role === "Admin") {
				...
				if (newArray.length !== 0) {
					const result = await axios.post(
						`${process.env.REACT_APP_API_URL}/Admin/getStudentsAllData`,
	...
	const handleDeleteStudent = async (data) => {
		...
		// 	try {
		// 		const result = await axios.post(
		// 			`${process.env.REACT_APP_API_URL}/Admin/deleteStudent`,
		// 			requestData,
		// 			{
		// 				headers: {
		// 					'Content-Type': 'application/json',
				...
				<table className="table">
						...
						}</tr>
					{
						studentList && studentList.length !== 0 ?
							studentList.map((d) => (
								<tr key={d.id}>
									...
									<td className='tableBtnArea'>
										{
											(batch && batch.studentDataList && batch.studentDataList.length !== 0)
												?
												<>
							...
							:
							<tr>
								<td colSpan="5" style={cellStyle}><h1>No Student Found</h1></td>
							</tr>
					}
