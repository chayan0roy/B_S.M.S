...
export default function A_Add_Student() {
        ...
        if (token && role && role === "Admin") {
            try {
                const result = await axios.post(
                    `${process.env.REACT_APP_API_URL}/Admin/getAllStudentsData`,
    ...
    const handleApproveStudent = async (data) => {
        ...
        if (token && role && role === "Admin") {
            try {
                ...
                const response = await axios.post(`${process.env.REACT_APP_API_URL}/Admin/approveStudent`, formData, {
                    headers: {
                        'Content-Type': 'application/json'
                    }
                });
